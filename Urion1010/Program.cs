...
namespace Urion1010
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int numeroPecas1 = int.Parse(valores[1]);
            double valorPeca1 = double.Parse(valores[2],CultureInfo.InvariantCulture);

            valores = Console.ReadLine().Split(" ");

            int codPeca2 = int.Parse(valores[0]);
            int numeroPecas2 = int.Parse(valores[1]);
            double valorPeca2 = double.Parse(valores[2],CultureInfo.InvariantCulture);

            double total = valorPeca1 * numeroPecas1 + valorPeca2 * numeroPecas2;
